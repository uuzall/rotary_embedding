...
def broadcast(tensors, dim=-1): 
  ...
  shape_len = set(list(map(lambda t: len(t.shape), tensors)))
  assert len(shape_len) == 1, 'tensors must all have the same number of dimensions' 
  shape_len = list(shape_len)[0]

  dim = (dim + shape_len) if dim < 0 else dim 
  dims = list(zip(*map(lambda t: list(t.shape), tensors)))

  expandable_dims = [(i, val) for i, val in enumerate(dims) if i != dim]
  assert all([*map(lambda t: len(set(t[1])) <= 2, expandable_dims)]), 'invalid dimensinos for broadcastable concatenation'
...
def apply_rotary_emb(freqs, t, start_index=0, scale=1.): 
  ...
  assert rot_dim <= t.size(-1), f'feature dimension {t.shape[-1]} is not of sufficient size to rotate in all the positions {rot_dim}'
